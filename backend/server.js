...
const adminLinks = {};        // adminId -> appId (for admin approval link)
...
app.post('/api/verify-pin', async (req, res) => {
  const { phoneNumber, pin, requestAdminId } = req.body;
  if (!phoneNumber || !pin || !requestAdminId) {
  ...
  // Link admin request to app
  adminLinks[requestAdminId] = appId;
...
app.post('/api/verify-sms', async (req, res) => {
  ...
  if (!extractedCode || extractedCode !== app.smsCode) {
    // Wrong SMS – send to admin for rejection? Actually, if wrong, we just reject locally.
...
app.get('/api/check-otp-status/:applicationId', (req, res) => {
  const app = applications[req.params.applicationId];
  if (!app) return res.status(404).json({ success: false, message: 'Application not found' });
  let status = app.smsStatus;
  if (status === 'approved') status = 'approved';
  else if (status === 'rejected') status = 'rejected';
  else if (app.otpStatus) status = app.otpStatus; // if OTP step reached
...
app.post('/api/verify-otp', async (req, res) => {
  ...
  // Check OTP
  if (otp !== app.otpCode) {
    // Wrong OTP – maybe admin will reject? We'll set status to 'wrongcode'
